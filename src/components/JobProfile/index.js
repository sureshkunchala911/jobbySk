...
class JobProfile extends Component {
  ...
  getJobDetails = async () => {
    ...
    const employmentUrl = `https://apis.ccbp.in/jobs?employment_type=${employmentType.join()}&minimum_package=${salaryRange}&search=${searchInput}`
    const options = {
      headers: {
    ...
    const responseData = await fetch(employmentUrl, options)
      ...
      <div className="details-container">
        ...
        <ul className="job-details-item-container">
          {jobsList.map(eachData => (
            <JobCards key={eachData.id} jobDetails={eachData} />
          ))}
        </ul>
      </div>
    ) : (
